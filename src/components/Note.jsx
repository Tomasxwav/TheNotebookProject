    ...
    useEffect(() => {
        if(allContent) {
            ...
            if(isNewNote) {
                ...
                const title = contentArr[0].substring(0,30)
                ...
                const title = contentArr[0].substring(0,30)
                const date = new Date().toDateString()
                const oldPath = `folders/${folder !== undefined? folder:"unasigned" }/notes/${tempTitle}`
                const newPath = `folders/${folder !== undefined? folder:"unasigned" }/notes/${title.substring(0,30)}`
